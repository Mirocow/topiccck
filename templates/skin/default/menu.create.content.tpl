...
<div class="dropdown-create">
	...
	<ul class="dropdown-menu{if $oConfig->GetValue('view.skin')!='social' && $oConfig->GetValue('view.skin')!='developer'}-create{/if}" id="dropdown-create-menu" style="display: none">
...
{if $sMenuItemSelect=='topic'}
	...
	<ul class="nav nav-pills mb-30">
		...
		{foreach from=$aActiveTopicTypes item=oType}
			<li {if $oType->getTypeCandelete()}{if $sEvent==$oType->getTypeUrl()}class="active"{/if}{else}{if $sMenuSubItemSelect==$oType->getTypeUrl()}class="active"{/if}{/if}>
				<a href="{if $oType->getTypeCandelete()}{router page='type'}{$oType->getTypeUrl()}/{else}{router page=$oType->getTypeUrl()}{/if}add/">{$oType->getTypeTitle()|escape:'html'}</a>
			</li>
		{/foreach}
